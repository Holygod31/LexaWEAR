 ...
 *
 * Each entry holds:
 *   - [Entry.hex]     — the canonical 6-char uppercase hex stored on NFC tags
 *   - [Entry.nameRes] — string resource ID for the localised display name
 *   - [Entry.r/g/b]   — pre-computed RGB components for nearest-match calculations
 *
 * All colour-related code in the app (fragments, VisionAnalyzer, CareFragment)
 * must use this object — never hardcode hex strings or colour names elsewhere.
 *
 * Usage:
...
object ColorPalette {
     ...
     * [hex] is stored on NFC tags and used as the filter key — must be 6 uppercase chars.
     * [r], [g], [b] are pre-computed to avoid repeated Color.parseColor calls during
     * nearest-match searches across the full palette.
     */
    data class Entry(
        val hex: String,
        val nameRes: Int,
    ...
    /**
     * The full ordered palette list. Order does not affect correctness but grouping
     * by hue family makes it easier to verify coverage and spot duplicates.
     * Adding or removing entries here automatically affects all carousels,
     * dropdowns, and nearest-match lookups throughout the app.
     */
     ...
     *
     * Callers should fall back to [nearestEntryFromArgb] for legacy tags that
     * stored an off-palette hex — that path returns a name but flags the value
     * as legacy in [CareFragment] and [WardrobeFragment].
     *
     * [getString] is passed in rather than called directly because this object
     * has no Context; callers supply `::getString` from their fragment or activity.
     */
    ...
    /**
     * Returns the palette entry whose RGB colour is closest to ([r], [g], [b])
     * using Euclidean distance in RGB space.
     *
     * Used by [VisionAnalyzer] to map a detected dominant colour to the nearest
     * palette entry. Also used by fragments to resolve off-palette legacy hex values.
     * Falls back to the last entry (multicolor) only if [entries] is somehow empty.
     */
    fun nearestEntry(r: Int, g: Int, b: Int): Entry {
        return entries.minByOrNull { entry ->
            ...
            sqrt(dr * dr + dg * dg + db * db)  // Euclidean distance — no sqrt optimisation
            // needed here; palette is small (~45 entries)
        } ?: entries.last()
    }

    /**
     * Convenience overload: unpacks a packed ARGB int (e.g. from [android.graphics.Color]
     * or a [android.graphics.Bitmap] pixel) and delegates to [nearestEntry].
